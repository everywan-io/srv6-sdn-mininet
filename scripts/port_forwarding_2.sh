...
# IP address of the interface facing the public net
INTERFACE_IP=${IPS[$INPUT_INTERFACE]}

# Add the port forwarding rule
iptables -t nat -I PREROUTING 1 -i $INPUT_INTERFACE -p udp --dport 20000 -j DNAT --to $EDGE:4789
iptables -t nat -I POSTROUTING 1 -o $INPUT_INTERFACE -p udp --source $EDGE --sport 4789 -j SNAT --to-source $INTERFACE_IP:20000
