...
iptables -t filter -A FORWARD -i $int_interface -o $ext_interface -m conntrack --ctstate NEW,ESTABLISHED,RELATED -j ACCEPT
iptables -t filter -A FORWARD -i $ext_interface -o $int_interface -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT
